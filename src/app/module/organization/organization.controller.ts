...
const getJoinOrganization = catchAsync(async (req: Request, res: Response) => {
  const user = req.user!;

  const organizationId = req.params.organizationId;

  const result = await organizationService.getJoinOrganization(
    user,
    organizationId as string,
  );
  sendResponse(res, {
...
const getMyJoinOrganization = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user!;

    const organizationId = req.params.organizationId;

    const result = await organizationService.getMyJoinOrganization(
      user,
      organizationId as string,
    );
    sendResponse(res, {
...
const updateOrganizationMember = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user;
    ...
    const result = await organizationService.updateOrganizationMember(
      user as RequestUser,
      organizationId as string,
...
const getOrganizationMember = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.user;

    const organizationId = req.params.organizationId;

    const result = await organizationService.getOrganizationMember(
      user as RequestUser,
      organizationId as string,
      req.params?.memberId as string,
    );
